...
if __name__ == "__main__":
    placeholder = subprocess.Popen(["rofi","-dmenu","-p",":Looking for Networks","-location","3","-width","23","-lines","0"])
    ...
    s = ""
    rofi = subprocess.Popen(["rofi","-dmenu","-p","Wifi","-location","3","-width","26","-lines",str(len(networks))],stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
    i=0
    for n in networks:
        s+=str(n)+" ("+str(i)+")\n"
        i+=1
